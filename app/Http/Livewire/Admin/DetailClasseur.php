...
class DetailClasseur extends Component
{
    use WithPagination;
    ...
    public $departement_all=[];
    public $divisions=[''];
    public $classeurs=[''],$deps=[];
    public $roles=[''];
    public $classeur=[''],$update;
    public function mount(){
        $this->departement_all = Departement::all();
        $this->deps = Departement::where('delete_at',null)->get();
        dd($this->deps);
        $this->roles= Role::all();
    ...
    {
        return view('livewire.humanres.detail-classeur',[
