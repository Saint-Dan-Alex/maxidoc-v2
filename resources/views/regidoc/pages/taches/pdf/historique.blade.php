...
<head>
    ...
    <title>Historique de la tâche #{{ $tache->id }}</title>
    <style>
        body {
...
<body>
    <div class="header">
        ...
        <div class="header-content">
            <h1>HISTORIQUE DE LA TÂCHE #{{ $tache->id }}</h1>
        ...
        <div class="details-container">
            ...
            <div class="details-column">
                <div class="info-item">
                    ...
                    <div class="info-value">{{ $tache->titre ?? 'N/A' }}</div>
                </div>
                <div class="info-item">
                    <div class="info-label">Date de création</div>
                    <div class="info-value">{{ $tache->created_at->format('d/m/Y H:i') }}</div>
                </div>
                <div class="info-item">
                    <div class="info-label">Priorité</div>
                    <div class="info-value">{{ $tache->priorite->titre ?? 'N/A' }}</div>
                </div>
            </div>
            ...
            <div class="details-column">
                <div class="info-item">
                    ...
                    <div class="info-value">{{ $tache->tache_statut->titre ?? 'N/A' }}</div>
                </div>
                <div class="info-item">
                    <div class="info-label">Progression</div>
                    <div class="info-value">{{ $tache->pourcentage }}%</div>
                </div>
                <div class="info-item">
                    <div class="info-label">Date d'échéance</div>
                    <div class="info-value">{{ $tache->date_fin ? $tache->date_fin->format('d/m/Y H:i') : 'Non définie' }}</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="info-section">
        ...
        @if($tache->historiques->count() > 0)
            <table class="history-table">
                ...
                <tbody>
                    @foreach($tache->historiques as $historique)
                        <tr>
                            <td>{{ $historique->created_at->format('d/m/Y H:i') }}</td>
                            <td>{{ $historique->user->agent->prenom . ' ' . $historique->user->agent->nom ?? 'Système' }}</td>
        ...
        @else
            <p>Aucune activité enregistrée pour cette tâche.</p>
        @endif
    </div>
