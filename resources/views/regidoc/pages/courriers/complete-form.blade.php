                ...
                <div class="form-group row g-3">
                            ...
                            <div class="col-7">
                                <select class="form-select form-control select autreSelect2"
                                    ...
                                    @foreach ($types as $type)
                                        @if ($type->id != 2)
                                            <option value="{{ $type->id }}" @selected(isset($courrier) && $courrier->type_id == $type->id)>
                                                {{ $type->titre }}
                                            </option>
                                        @endif
                                    @endforeach
                        ...
                        <div class="row">
                            ...
                            <div class="col-7" wire:ignore>
                                <select class="form-select form-control select2" aria-label="Default select example"
                                    ...
                                    @if(isset($courrier) && $courrier->categorie)
                                        <option value="{{ $courrier->categorie->id }}" selected>{{ $courrier->categorie->title }}</option>
                                    @endif
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-7">
                                <select class="form-select form-control mb-1 select2"
                                    ...
                                    data-placeholder="Selectionnez" @if (isset($isConfidentiel) && $isConfidentiel) disabled @endif
                                    ...
                                    @if(isset($courrier) && $courrier->externDestinateur)
                                        <option value="{{ $courrier->externDestinateur->id }}" selected>{{ $courrier->externDestinateur->nom }}</option>
                                    @endif
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-7">
                                <select class="form-select form-control selectCopie"
                                    ...
                                    @foreach ($directions as $direction)
                                        <option value="{{ $direction->id }}" @selected(isset($courrier) && $courrier->dest_interne_id == $direction->id)>
                                            {{ $direction->titre }}
                                        </option>
                                    @endforeach
                    ...
                    <div class="col-12 datearrive_field" wire:ignore>
                        <div class="row">
                            ...
                            <div class="col-7">
                                <input type="datelocal-time" class="form-control" id="inputPassword1" disabled name="date-arriv" value="{{ isset($courrier->date_arrive) ? $courrier->date_arrive : '' }}"
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function setEntrat(data) {
                // Envoi d'une requête AJAX pour mettre à jour la référence
                $.ajax({
                    url: '{{ route("regidoc.courriers.change-num-ref") }}',
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        type_id: data
                    ...
                    success: function(response) {
                        if (response.reference) {
                            $('input[name="ref"]').val(response.reference);
                        }
                    }
                });
            ...
            $('select[name="service_init"]').on('change', function(e) {
                ...
                // Par exemple, vous pouvez déclencher un événement personnalisé
                $(document).trigger('serviceInitChanged', [serviceId]);
                
                // Si vous avez besoin d'effectuer une action côté serveur, vous pouvez ajouter une route et une méthode de contrôleur ici
                // puis décommenter et adapter ce code :
                /*
                $.ajax({
                    url: '/votre-route-pour-mise-a-jour-service',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                    ...
                    error: function(xhr) {
                        console.error('Erreur lors de la mise à jour du service initialisateur');
                    }
                });
                */
            });
            ...
            // Vérifier la validité du formulaire à chaque modification d'un champ obligatoire
